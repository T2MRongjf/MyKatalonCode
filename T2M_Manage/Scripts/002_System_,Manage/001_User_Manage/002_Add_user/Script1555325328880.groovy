...
try {
    a = findTestObject('Syetem_settings/User_Manage/Search/td_number1')

    data_content = WebUI.getAttribute(findTestObject('Syetem_settings/User_Manage/Search/td_number1'), 'data-content')

    WebUI.comment(data_content)

    if (data_content == username) {
        WebUI.comment('列表中搜索到该用户，人员添加成功')
    } else {
        WebUI.comment('列表中没有搜索到该用户，人员未添加成功')

        assert false
    }
}
catch (def NoSuchElementException) {
    WebUI.comment(('脚本中添加的人员姓名为' + username) + ',人员未添加成功，列表中不存在该元素，请检查！！')
